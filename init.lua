...
-- Clipboard content that an app flags as concealed or transient is never
-- recorded, so passwords copied from a password manager or the macOS
-- Passwords app stay out of the history.
local concealedTypes = {
	["org.nspasteboard.ConcealedType"] = true, -- password managers mark secrets
	["org.nspasteboard.TransientType"] = true, -- explicitly not for history
	["org.nspasteboard.AutoGeneratedType"] = true, -- generated, don't persist
	["com.agilebits.onepassword"] = true, -- 1Password's own concealed marker
}

local function pasteboardIsConcealed()
	local uti = hs.pasteboard.contentTypes and hs.pasteboard.contentTypes()
	if type(uti) == "table" then
		for _, t in ipairs(uti) do
			if concealedTypes[t] then
				return true
			end
		end
	end
	local types = hs.pasteboard.typesAvailable()
	for t in pairs(concealedTypes) do
		if types[t] then
			return true
		end
	end
